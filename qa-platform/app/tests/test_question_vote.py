...
from app.views import QuestionVoteView
...
class QuestionVoteTest(TestCase):
    def setUp(self):
        ...
        self.vote_url = reverse('app:question_vote', kwargs={'pk': self.question.pk})
        self.detail_url = reverse('app:question_detail', kwargs={'pk': self.question.pk})

    def test_view_is_class_based(self):
        self.assertTrue(issubclass(QuestionVoteView, QuestionVoteView.__mro__[0]))

    def test_authenticated_user_can_upvote_question(self):
    ...
    def test_authenticated_user_can_downvote_question(self):
        self.client.login(username='voter1', password='password123')
        response = self.client.post(self.vote_url, {'value': '-1'})
        self.assertRedirects(response, self.detail_url)
    ...
    def test_user_cannot_create_multiple_votes_for_same_question(self):
        self.client.login(username='voter1', password='password123')
        self.client.post(self.vote_url, {'value': '1'})
    ...
    def test_question_score_calculated_correctly_with_multiple_voters(self):
        self.client.login(username='voter1', password='password123')
        self.client.post(self.vote_url, {'value': '1'})
        ...
        self.assertEqual(self.question.score, 2)
        self.assertEqual(self.question.upvotes_count, 2)
        self.assertEqual(self.question.downvotes_count, 0)
    ...
    def test_vote_associated_with_correct_question_and_user(self):
        ...
        self.assertEqual(vote.content_object, self.question)
        self.assertNotEqual(vote.content_object, other_question)
        self.assertEqual(vote.user, self.voter1)
        self.assertEqual(self.question.score, 1)
        self.assertEqual(other_question.score, 0)
    ...
    def test_existing_question_detail_answer_and_comment_functionality_works(self):
        # Ensure answers and comments can still be posted and viewed alongside voting
        ...
        response = self.client.get(self.detail_url)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'This is a helpful answer.')
        self.assertContains(response, 'This is a question comment.')
